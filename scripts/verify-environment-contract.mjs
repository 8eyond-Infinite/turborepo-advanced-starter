...
const keysFrom = (content) => new Set(content.split(/\r?\n/).map((line) => line.match(/^([A-Z][A-Z0-9_]*)\s*=/)?.[1]).filter(Boolean));
const errors = [];
for (const contract of contracts) {
  try {
    const keys = keysFrom(readFileSync(resolve(root, contract.file), 'utf8'));
    for (const key of contract.keys) if (!keys.has(key)) errors.push(`${contract.file}: missing ${key}`);
  } catch { errors.push(`${contract.file}: file is missing or unreadable`); }
}
if (errors.length) {
...
console.log(`Environment contract verified: ${contracts.length} example files.`);
