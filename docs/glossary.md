...
| Thuật ngữ                  | Hiểu đơn giản là                                                                                                                       | Trong repo này                                                                                                   |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Migration                  | File SQL được đánh số thứ tự, ghi lại LỊCH SỬ thay đổi schema. Môi trường mới dựng DB bằng cách chạy lại toàn bộ lịch sử.              | `packages/database/prisma/migrations/` — 7 migration, chạy bằng `prisma migrate deploy`.                         |
...
| Metrics / Prometheus       | Số liệu đo được (đếm, thời lượng) mà hệ thống giám sát kéo về định kỳ qua một endpoint text.                                           | `GET /metrics`: HTTP latency, outbox lag và `bullmq_jobs{queue,status}`; production dùng Bearer `METRICS_TOKEN`. |
