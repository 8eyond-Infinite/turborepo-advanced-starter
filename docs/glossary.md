...
| Metrics / Prometheus       | Số liệu đo được (đếm, thời lượng) mà hệ thống giám sát kéo về định kỳ qua một endpoint text.                                           | `GET /metrics`: `http_request_duration_seconds`, `outbox_events{status}`, `outbox_oldest_pending_age_seconds`. |
