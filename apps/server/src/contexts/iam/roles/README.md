...
│   └── controllers/
│       └── roles.controller.ts                  # REST API Controller điều hướng và bảo vệ quyền truy cập
```
...
* **Hoạt động**:
  1. Hứng request HTTP.
  2. Sử dụng decorator `@RequirePermissions('role:write')` để chắc chắn chỉ Admin có quyền sửa mới được đi tiếp.
